...
bool check_collapse(int board[][BOARD_SIZE], int i, int j, int ans){
    ...
    //check if s collapse
    int I = (i/3)*3+1, J = (j/3)*3+1;
    if(board[I][J-1] != ans && board[I][J] != ans && board[I][J+1] != ans){
        if(board[I-1][J-1] != ans && board[I-1][J] != ans && board[I-1][J+1] != ans){
            if(board[I+1][J-1] != ans && board[I+1][J] != ans && board[I+1][J+1] != ans){
                return true;
            }
        }
...
bool solve(int board[][BOARD_SIZE], int i, int j){
    cout << "now in (i,j) = (" << i << "," << j << ")" << endl;
    if(board[i][j] != 0){
        if(i<BOARD_SIZE && j<BOARD_SIZE-1){
            if(!solve(board, i, j+1)){
                cout << " return false now in (i,j) = (" << i << "," << j << ") with ans = " << board[i][j] << endl;
                return false;
            }
        }
        ...
        for(int ans = 1; ans <= 9; ans++){
            ...
            //j not to the end
            if(i<BOARD_SIZE && j<BOARD_SIZE-1){
                if(check_collapse(board, i, j, ans)){
                    board[i][j] = ans;
                    cout << "now call (" << i << "," << j+1 << ")" << endl;
                    if(solve(board, i, j+1))
                        break;
                    else
                        board[i][j] = 0;
    ...
    //we did it!
    cout << " return true now in (i,j) = (" << i << "," << j << ") with ans = " << board[i][j] << endl;
    return true;
}
